...
const PlayerGeometry:THREE.BufferGeometry|Promise<THREE.BufferGeometry> = loader.loadAsync('./models/ply/steve.ply');

@ASerializable("PushBoxPlayerNodeModel")
export class PushBoxPlayerNodeModel extends ALoadedModel{
    //Our vertices
    static PlayerObject3D:THREE.Object3D;
    ...
    static async CreateDefaultNode(radius:number=50, widthSegments:number=50, heightSegments:number=50, ...args:any[]){
        if(!PushBoxPlayerNodeModel.PlayerObject3D){
            const geometry = await PlayerGeometry;
            PushBoxPlayerNodeModel.PlayerObject3D = new THREE.Mesh(geometry);
        }
        let newNode = new PushBoxPlayerNodeModel()
        newNode.setMaterial("steve");
        return newNode;
    }
